...
func get_interaction_data() -> InteractionData:
    ...
    var interactable_name = tr("Trash Can")
    var desc = tr("A trash can. You can throw away items here.")
    if PlayerInventorySingleton.has_item():
        var held_item = PlayerInventorySingleton.held_item_data()
        action = InteractionAction.new(tr("Throw Away %s") % held_item.item_name, interact)
        desc = tr("A trash can. Throw away the %s here.") % held_item.item_name.to_lower()
